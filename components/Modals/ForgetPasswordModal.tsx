...
import { changeModalAction, loginErrors, user_modals } from '../../app/containers/AuthSlice';
interface Props {
}
...
function ForgetPasswordModal({}: Props):ReactElement {
    ...
    const [forgetPassword, setforgetPassword] = useState({email:"", name:"", password:""})
    ...
    const formSubmit = async (e:FormEvent) => {
        e.preventDefault()
        dispatch(authThunk.userLogin(forgetPassword))
    }
        ...
        <div style={{width:"100vw", height:"100vh" ,  display: 'flex', justifyContent: 'center', alignItems: 'center', position:'absolute', background:"rgba(0,0,0,0.5)", left:0,top:0,zIndex:4}}>
            <form style={{background:"rgba(255,255,255,0.7)" , padding:"25px", borderRadius:"20px"}} onSubmit={formSubmit}>
                ...
                <div style={{display:'flex',flexDirection:"column",alignItems:'center',marginTop:"20px",marginBottom:"10px"}}>
                    ...
                    <input type="email" name="email" value={forgetPassword.email} onChange={formChange}/>
                </div>
                <div style={{display:'flex',flexDirection:"column",alignItems:'center',marginTop:"20px",marginBottom:"10px"}}>
